...
try {
  ...
  const urgentTask = addTask(
    "Contact Sarah's Marketing Agency about overdue payment",
    "urgent",
    "customer",
    new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString(), // Due in 2 hours
    customer2.id
  );

  const shipmentTask = addTask(
    "Prepare bulk order for Tech Innovators",
    "high",
    "shipment",
  ...
  const followUpTask = addTask(
    "Schedule quarterly business review with Tech Innovators",
    "medium",
    "follow-up",
    new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), // Due in a week
    customer1.id
  );
