...
export async function getDashboardData() {
    const [pendingShipments, urgentTasks, customersNeedingAttention, recentOrders, totalRevenue] = await Promise.all([
...
export async function getTasksSummary() {
    const [total, incomplete, urgent, overdue, today] = await Promise.all([
        prisma.task.count(),
...
export async function getShipmentsSummary() {
    const [allShipments, pendingCount, overdueCount] = await Promise.all([
        getShipments(),
...
export async function getCustomersSummary() {
    const [customers, customerCount, needingAttentionCount] = await Promise.all([
...
export async function getPersonalizedDailyTasks() {
    ...
    const tomorrow = new Date(today);
    tomorrow.setDate(tomorrow.getDate() + 1);
    const [urgent, overdue, todayTasks] = await Promise.all([
...
export async function getAttentionItems() {
    const [overdueShipments, urgentTasks, overdueTasks] = await Promise.all([
        getOverdueShipments(),
        prisma.task.findMany({ where: { priority: "HIGH" } }),
        prisma.task.findMany({
            where: {
...
export async function generateDailyReport() {
    const [dashboard, tasksSummary, shipmentsSummary, attentionItems] = await Promise.all([
        getDashboardData(),
        getTasksSummary(),
        getShipmentsSummary(),
        getAttentionItems()
